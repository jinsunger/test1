...
public class ItemOrderController extends BaseController {
    ...
    @Autowired
    private ShopCartService carService;
    @Autowired
    ...
    @ResponseBody
    public String exAdd(@RequestBody List<ShopCartDto> list, HttpServletRequest request){
        Object attribute = request.getSession().getAttribute(Consts.USERID);
        ...
        for(ShopCartDto c:list){
            ids.add(c.getId());
            ShopCart load = carService.load(c.getId());
